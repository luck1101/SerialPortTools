...
JNIEXPORT jobject JNICALL Java_android_serialport_api_SerialPort_open
        (JNIEnv *env, jobject thiz, jstring path, jint baudrate, jint databits, jint stopbits, jchar parity) {
    int fd;
    speed_t speed;
    ...
    {
        serial_set_parity(fd, databits, stopbits, parity);
    }
...
(JNIEnv *env, jobject thiz, jobject mFd) {
    jclass FileDescriptorClass = (*env)->GetObjectClass(env, mFd);
